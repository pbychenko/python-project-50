...
def test_json():
    ...
    assert generate_diff(file1_path, file2_path, 'json') == json.dumps(result_data)
...
def test_yaml():
    ...
    result_path = os.path.join(os.getcwd(), 'tests', 'fixtures', 'yaml', 'result.json')
    result_data = json.load(open(result_path, 'r'))

    assert generate_diff(file1_path, file2_path, 'yaml') == json.dumps(result_data)
